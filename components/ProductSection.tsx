    ...
    <div className="w-full max-w-7xl mx-auto">
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-1.5 md:gap-3">
        {products.map((p) => {
          ...
          return (
            // Changed rounded-2xl to rounded-lg for a cleaner, flush design
            <div key={p.id} className="group flex flex-col bg-white dark:bg-[#151515] rounded-lg border border-slate-200 dark:border-slate-800 overflow-hidden shadow-sm hover:shadow-lg transition-all h-full relative">

              <Link href={`/product/${p.publicId || p.id}`} className="flex flex-col flex-grow">
                {/* Image Area - Ensure rounded-t-lg matches the parent container */}
                ...
                <div className="p-2.5 flex flex-col flex-grow">
                  <h3 className="text-[13px] md:text-sm font-bold text-slate-900 dark:text-slate-100 line-clamp-2 leading-tight mb-1.5 group-hover:text-[#D97706] transition-colors">{p.name}</h3>
              ...
              <div className="grid grid-cols-4 border-t border-slate-100 dark:border-slate-800 bg-slate-50 dark:bg-[#111]">
                 ...
                 >
                   ...
                   {/* Hides the text on extra small screens so the button doesn't stretch */}
