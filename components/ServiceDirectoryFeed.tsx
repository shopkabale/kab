...
export default function ServiceDirectoryFeed({ initialProducts }: { initialProducts: Product[] }) {
  ...
  if (!products || products.length === 0) {
    return null; // The fallback UI in the parent page will handle the empty state
  }
  ...
  return (
    <div className="w-full">
      ...
      {/* DIRECTORY GRID (1 Column Mobile, 2 Columns Desktop) */}
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-6">
        {products.map((service) => {
          ...
          return (
            // Notice we are linking to `/service/[id]`, which we will build in Phase 3!
            <Link 
            ...
            >
              {/* LANDSCAPE IMAGE BOX (16:9 Aspect Ratio) */}
              <div className="relative w-full aspect-video bg-slate-50 dark:bg-[#111] border-b border-slate-100 dark:border-slate-800 p-2 sm:p-4">
                <Image
                  src={mainImage}
                  alt={service.title}
                  fill
                  sizes="(max-width: 768px) 100vw, 50vw"
                  // object-contain ensures text on flyers/posters is never cropped!
