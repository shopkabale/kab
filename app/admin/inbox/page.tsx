...
function InboxContent() {
  ...
  // 1. Listen to all WhatsApp messages in real-time
  useEffect(() => {
      ...
      <div className="bg-slate-900 rounded-3xl p-8 mb-8 text-white flex flex-col md:flex-row md:items-center justify-between shadow-xl">
        ...
        <div className="mt-6 md:mt-0 flex gap-4">
          <div className="bg-slate-800 px-6 py-3 rounded-2xl border border-slate-700 text-center">
            <span className="block text-2xl font-black text-[#D97706]">{messages.length}</span>
            <span className="text-[10px] uppercase font-bold text-slate-500">Total Chats</span>
          </div>
        </div>
      </div>
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-slate-50 border-b border-slate-100">
                  <th className="p-5 text-xs uppercase font-black text-slate-500 tracking-wider">Customer</th>
                    ...
                    <td className="p-5">
                      <div className="flex items-center gap-3">
                        <div className="w-10 h-10 bg-amber-100 text-[#D97706] rounded-full flex items-center justify-center font-bold">
                          {msg.senderPhone?.substring(msg.senderPhone.length - 2)}
                        </div>
                        <span className="font-bold text-slate-900">+{msg.senderPhone}</span>
                      </div>
                    </td>
                    <td className="p-5">
