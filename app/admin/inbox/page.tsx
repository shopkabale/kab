...
function InboxContent() {
  ...
  // 1. Fetch the list of chats and sort them in JavaScript so we don't lose old ones
  useEffect(() => {
    if (!user || user.role !== "admin") return;

    // Fetching everything without orderBy so Firestore doesn't hide missing fields
  ...
  return (
    <div className="max-w-7xl mx-auto py-8 px-4 sm:px-6 h-screen flex flex-col">
      ...
      {/* Split Screen Layout - Wrapped in overflow-x-auto for mobile desktop view */}
      <div className="flex-1 overflow-x-auto pb-4">
        {/* min-w-[800px] ensures it never compacts/squishes, forcing horizontal scroll on small screens */}
        <div className="flex gap-6 min-w-[800px] h-full min-h-[600px]">
          
          {/* LEFT PANEL: Chat List */}
          <div className="w-1/3 bg-white rounded-3xl border border-slate-200 shadow-sm flex flex-col overflow-hidden h-full">
          ...
          <div className="w-2/3 bg-slate-50 rounded-3xl border border-slate-200 shadow-sm flex flex-col overflow-hidden h-full">
            {!selectedChatId ? (
