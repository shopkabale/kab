...
export default async function ProductDetailsPage({ params }: { params: { publicId: string } }) {
  ...
  return (
    <div className="py-8 max-w-6xl mx-auto px-4 sm:px-6">
      ...
      {/* Breadcrumb Navigation (FIXED: Now points to dynamic /category/ route) */}
      <div className="mb-6 flex items-center text-sm text-slate-500 font-medium overflow-x-auto whitespace-nowrap scrollbar-hide">
        ...
        <Link href={`/category/${safeCategory}`} className="hover:text-primary transition-colors capitalize">
      ...
      <div className="bg-white rounded-3xl shadow-sm border border-slate-200 overflow-hidden">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-0 lg:gap-8 p-6 lg:p-8">
          ...
          {/* Left Column: Lighthouse Image Gallery */}
          ...
          <div className="flex flex-col h-full">
            ...
            <div className="bg-slate-50 border border-slate-100 rounded-xl p-4 mb-8 flex items-center gap-4">
              <div className="w-12 h-12 bg-sky-100 text-sky-700 rounded-full flex items-center justify-center font-bold text-xl flex-shrink-0">
            ...
            <div className="mt-auto border-t border-slate-100 pt-6 space-y-4">
              <Link 
                ...
                className="w-full bg-primary text-white py-4 rounded-xl font-bold text-lg hover:bg-sky-500 transition-all hover:shadow-lg hover:-translate-y-1 flex items-center justify-center gap-2"
              >
                🛒 Proceed to Checkout
              </Link>
              
              {/* If you have extra actions like 'Save for later', they render here */}
