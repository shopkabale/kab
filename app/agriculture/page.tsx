...
export const metadata = {
  title: "Agriculture | Kabale Online",
  description: "Fresh produce, seeds, and farming equipment in Kabale.",
};

export default async function AgriculturePage() {
  const products = await getProducts("agriculture");
    ...
    <div className="py-8">
      <div className="mb-8 border-b border-slate-200 pb-4">
        ...
        <p className="mt-2 text-lg text-slate-600">
          Support local farmers. Buy fresh produce, tools, and supplies.
        </p>
      </div>

      {products.length === 0 ? (
        <div className="text-center py-12 bg-white rounded-lg border border-dashed border-slate-300">
          <h3 className="text-lg font-medium text-slate-900">No agriculture products found</h3>
          <p className="mt-1 text-sm text-slate-500">
            Check back soon! Local farmers are adding new items.
          </p>
        </div>
      ) : (
