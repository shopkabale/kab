...
export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
  ...
  const productEntries = productsSnap.docs.map((doc) => ({
    url: `${baseUrl}/item/${doc.data().publicId || doc.id}`,
  ...
  const blogSnap = await adminDb.collection("blog_posts").orderBy("publishedAt", "desc").limit(30).get();
  ...
  return [...staticPages, ...categoryEntries, ...productEntries, ...blogEntries] as MetadataRoute.Sitemap;
}